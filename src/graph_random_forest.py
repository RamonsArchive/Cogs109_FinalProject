"""
Random Forest Model Visualization Functions
Saves plots to plots/random_forest/ directory
"""
...
def get_random_forest_report_path(model_name: str, is_best: bool = False):
    """Generate path for text report"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    base_dir = Path("plots") / "random_forest"
...
def get_random_forest_plot_path(model_name: str, plot_type: str, is_best: bool = False):
    """
    Generate plot path for Random Forest models
    
    Args:
        model_name: Name of the model (e.g., "random_forest_count")
...
def graph_random_forest_model(
    model_results: dict,
    model_name: str,
    is_best: bool = False,
):
    """
    Generate comprehensive visualizations for Random Forest model results
    ...
    n_estimators = model_results.get("n_estimators", 100)
    max_depth = model_results.get("max_depth", None)
    min_samples_split = model_results.get("min_samples_split", 2)
    min_samples_leaf = model_results.get("min_samples_leaf", 1)
    max_features = model_results.get("max_features", "sqrt")
    ...
    max_depth_str = "None" if max_depth is None else str(max_depth)
    model_type_label = "Random Forest (Log-Transformed Target)" if is_log_target else "Random Forest (Count Data)"
    plt.suptitle(
        f"{title_prefix}Random Forest Model Performance: {display_name} {model_type_label}\n"
        f"n_estimators={n_estimators}, max_depth={max_depth_str}, min_samples_split={min_samples_split}, "
        f"min_samples_leaf={min_samples_leaf}, max_features={max_features}",
        fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
    
    filename1 = get_random_forest_plot_path(model_name, "metrics", is_best)
    ...
    plt.tight_layout()
    filename2 = get_random_forest_plot_path(model_name, "scatter", is_best)
    ...
    filename3 = get_random_forest_plot_path(model_name, "residuals", is_best)
    ...
    if feature_importances is not None and feature_names is not None:
        ...
        plt.tight_layout()
        filename4 = get_random_forest_plot_path(model_name, "feature_importance", is_best)
    ...
    plt.tight_layout()
    filename5 = get_random_forest_plot_path(model_name, "error_by_count", is_best)
    plt.savefig(filename5, dpi=300, bbox_inches="tight")
    print(f"✓ Saved: {filename5}")
    plt.close()
    
    # ========== 6. GENERATE TEXT REPORT ==========
    report = generate_random_forest_report(
        display_name,
        model_results,
        y_test,
        y_pred,
    ...
    report_file = get_random_forest_report_path(model_name, is_best)
    with open(report_file, "w") as f:
        f.write(report)
    
    print(f"\n{'='*70}")
    print(f"All visualizations saved to: plots/random_forest/")
    print(f"✓ Report saved: {report_file}")
    print(f"{'='*70}\n")


def generate_random_forest_report(
    display_name: str,
    model_results: dict,
    y_test: np.ndarray,
    y_pred: np.ndarray,
    ...
    """Generate detailed text report for Random Forest models"""
    ...
    max_depth_str = "None (unlimited)" if max_depth is None else str(max_depth)
...
HYPERPARAMETERS:
  • n_estimators (trees):        {n_estimators}
...
"""
    ...
    if is_best:
        ...
        report += f"""
{'='*70}
🎯 This is the BEST Random Forest model based on 10-fold CV {primary_metric}
